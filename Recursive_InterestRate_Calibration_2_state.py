...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  3 11:56:13 2021
...
alpha=[0.81, 0.67]
gamma = [0.009, 0.032]
nu = [1- 3*0.0105, 1- 6*0.0105]
fish_alpha = [0,0]
fish_gamma=[0,0]
fish_nu=[0,0]

Pi = np.array([[1/2,1/2],[1/2,1/2]])
...
for u in range(0,no_passes):
    ...
    for k in range(0,batch+1):#batch+1
        ...
        sigmaXk[:,inta+k-1]  = Gamma[k,0] *sigmaX[0,inta+k-2]*Pi[:,0] + Gamma[k,1] * sigmaX[1,inta+k-2]*Pi[:,1] 
        ...
        for r in range(0,state):
            sumJ1[:,r]= Gamma[k,0]*sigmaJ1[0,r]*Pi[:,0]+ Gamma[k,1] *sigmaJ1[1,r]*Pi[:,1] 
        ...
        for r in range(0,state):
            sumJ2[:,r]= Gamma[k,0] *sigmaJ2[0,r]*Pi[:,0]+Gamma[k,1] *sigmaJ2[1,r]*Pi[:,1] 
        ...
        for r in range(0,state):
            sumO[:,r]= Gamma[k,0] *sigmaO[0,r]*Pi[:,0]+Gamma[k,1] *sigmaO[1,r]*Pi[:,1] 
            sigmaO[:,r]= sumO[:,r] + Gamma[k,r]*sigmaX[r,inta+k-2]*Pi[:,r]
        sumOa= sum(sigmaO)/sumX[inta+k-2]
        remainderSO[:,inta+k-1]=sumOa.transpose()
        
        for r in range(0,state):
            sumT1[:,r]= Gamma[k,0] *sigmaT1[0,r]*Pi[:,0]+Gamma[k,1] *sigmaT1[1,r]*Pi[:,1] 
        ...
        for r in range(0,state):
            sumT2[:,r]= Gamma[k,0] *sigmaT2[0,r]*Pi[:,0]+Gamma[k,1] *sigmaT2[1,r]*Pi[:,1] 
        ...
        for r in range(0,state):
            sumT3[:,r]= Gamma[k,0] *sigmaT3[0,r]*Pi[:,0]+Gamma[k,1] *sigmaT3[1,r]*Pi[:,1] 
    ...
    for i in range(0,state):
        a[i]= -np.log((alpha[i]))/(u+1-u)
        beta[i]= gamma[i]/(1-alpha[i]) # aplha must (-) removed
        theta[i]= nu[i]* np.sqrt(np.abs(2*a[i]/(a[i]-np.exp(-2*a[i]*(u+1-u)))))
    ...
    for r in range(0,state):
        nu[r] = math.sqrt(abs(remainderST2[r,inta+batch-1]+ (alpha[r]**2) * remainderST2[r,inta+batch-2] + (gamma[r]**2) * remainderSO[r,inta+batch-2] - 2*alpha[r]*remainderST3[r,inta+batch-2]- 2*gamma[r]*remainderST1[r,inta+batch-1] - 2*alpha[r]*gamma[r]* remainderST1[r,inta+batch-2])/remainderSO[r,inta+batch-2])
    for r in range(0,state):
...
for i in range(0,no_passes):
    ...
    AIC3st[i]= -2*LogLikelihoodTotal[i]+2*6
    BIC3st[i]= -2*LogLikelihoodTotal[i]+6*math.log(batch)
